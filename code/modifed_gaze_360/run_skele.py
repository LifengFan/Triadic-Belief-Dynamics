...
source_path = './3d_pose2gaze/' # path storing head bounding box
val_file = "gaze360_validate_data_skele.txt"
train_file = "gaze360_training_data_skele.txt"
test_file = "gaze360_test_data_skele.txt"
...
test = False
checkpoint_test = './model_best_Gaze360_after_norm.pth.tar'
...
def main():
    ...
    # checkpoint = torch.load('./model_best_Gaze360_resfc1e4.pth.tar')
    checkpoint = torch.load('../gaze360_model.pth.tar')
    model.load_state_dict(checkpoint['state_dict'])
    model.module.base_model.fc2 = nn.Linear(1000, model_v.img_feature_dim)
    model.module.lstm = nn.LSTM(model_v.img_feature_dim + 78, model_v.img_feature_dim + 78, bidirectional=True, num_layers=2, batch_first=True)
    model.module.last_layer = nn.Linear(2*(model_v.img_feature_dim + 78), 3)
    model.cuda()
    ...
    image_normalize = transforms.Normalize(mean=[0.421, 0.371, 0.395], std=[0.185, 0.172, 0.183])
    ...
    # loader = torch.utils.data.DataLoader(ImagerLoader(source_path,train_file,transforms.Compose([
    #         transforms.RandomResizedCrop(size=224,scale=(0.8,1)),transforms.ToTensor()])),
...
def train(train_loader, model, criterion,optimizer, epoch):
    ...
    for i,  (source_frame, skeleton, target) in enumerate(train_loader):

        # measure data loading time
        data_time.update(time.time() - end)
        source_frame = source_frame.cuda(async=True)
        skeleton = skeleton.cuda(async=True)
        target = target.cuda(async=True)


        source_frame_var = torch.autograd.Variable(source_frame)
        skeleton_var = torch.autograd.Variable(skeleton)
        target_var = torch.autograd.Variable(target)

        # compute output
        output,ang_error = model(source_frame_var, skeleton_var)
...
def validate(val_loader, model, criterion):
    ...
    for i, (source_frame, skeleton, target) in enumerate(val_loader):
        ...
        skeleton_var = torch.autograd.Variable(skeleton, volatile=True)
