...
class mydataset(torch.utils.data.Dataset):
    def __init__(self, train_x, train_y):
        ...
        self.person_tracker_bbox = './tracker_record_bbox/'
        self.person_battery_bbox = './record_bbox/'
        self.obj_bbox = './post_neighbor_smooth_newseq/'
        self.cate_path = './track_cate/'
        self.feature_path = './feature_single/'
...
def main():
    ...
    train(learningRate, epochs, batch_size, train_x, train_y, validate_x, validate_y, './cptk/model_best.pth', 20)
    net = AttMat()
...
def test_data_seq(clip):
    ...
    attmat_path = './person_attention/'
    save_path = './save_mat/' + clip.split('.')[0] + '/'
...
def test_data_seq_get_obj_per_frame_raw_obj(clip, net):
    ...
    for frame_id in range(len(obj_records[obj_names[0]])):
        ...
        for obj_id in range(len(obj_names)):
            ...
            x_min = obj_bbox[0]
            y_min = obj_bbox[1]
            x_max = obj_bbox[0] + obj_bbox[2]
            y_max = obj_bbox[1] + obj_bbox[3]
