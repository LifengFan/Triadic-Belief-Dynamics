...
import units
import sys
sys.path.append('/home/shuwen/data/Six-Minds-Project/obj_oriented_event/')
from attention_model import *

class MindCNNNoHog(torch.nn.Module):
    def __init__(self):
        super(MindCNNNoHog, self).__init__()
...
class MLP_Feature(torch.nn.Module):
    def __init__(self):
        ...
        self.fc_event = torch.nn.Linear(1625, 1400)
        self.fc_event_2 = torch.nn.Linear(1400, 1200)
        self.fc_memory = torch.nn.Linear(1200, 1024)
    ...
    def forward(self, x):
        ...
        out = self.fc_memory(x2)


        return out

class MLP_Feature_Mem(torch.nn.Module):
    def __init__(self):
        super(MLP_Feature_Mem, self).__init__()
        self.fc_event = torch.nn.Linear(1625, 500)
        self.fc_event_2 = torch.nn.Linear(500, 50)
        self.fc_memory = torch.nn.Linear(60, 1024)
        self.fc_memory_2 = torch.nn.Linear(600, 1024)
    ...
    def forward(self, x, y):
        ...
        x1 = self.relu(self.fc_event(x))
        x2 = self.fc_event_2(x1)

        # input = torch.cat([x2, y], dim=1)
        # out = self.relu(self.fc_memory(input))
        # out = self.fc_memory_2(out)

        # x2 = self.fc_event(x)

        input = torch.cat([x2, y], dim=1)
        out = self.fc_memory(input)
...
class MLP_Combined(torch.nn.Module):
    def __init__(self):
        super(MLP_Combined, self).__init__()
        self.fc = torch.nn.Linear(16, 200)
        self.fc1 = torch.nn.Linear(200, 800)
        self.fc2 = torch.nn.Linear(800, 1024)
        self.relu = torch.nn.ReLU()
...
class MLP_Combined_Att(torch.nn.Module):
    def __init__(self):
        super(MLP_Combined_Att, self).__init__()
        self.fc = torch.nn.Linear(19, 200)
        self.fc1 = torch.nn.Linear(200, 800)
        self.fc2 = torch.nn.Linear(800, 1024)
        self.relu = torch.nn.ReLU()
