...
class mydataset_atomic_with_label(torch.utils.data.Dataset):
    def __init__(self, seq, label, args):
        ...
        self.head_num=2
        self.node_num=4
        self.img_path = args.img_path #'../annotations/'
        self.person_tracker_bbox = args.tracker_bbox # '../data/3d_pose2gaze/tracker_record_bbox/'
        self.person_battery_bbox = args.battery_bbox #'../data/3d_pose2gaze/record_bbox/'
        self.obj_bbox = args.obj_bbox # '../post_neighbor_smooth_newseq/'
        self.cate_path = args.cate_path # '../data/track_cate/'
...
def get_data(args):
    ...
    train_set = mydataset_atomic_with_label(test_seqs[:train_ratio], event_labels[:train_ratio], args)
    train_loader = torch.utils.data.DataLoader(train_set, collate_fn=collate_fn_atomic_fine_tune, batch_size=16,shuffle=False)
    validate_set = mydataset_atomic_with_label(test_seqs[train_ratio:train_ratio + validate_ratio], event_labels[train_ratio:train_ratio + validate_ratio], args)
    validate_loader = torch.utils.data.DataLoader(validate_set, collate_fn=collate_fn_atomic_fine_tune, batch_size=16, shuffle=False)
    test_set = mydataset_atomic_with_label(test_seqs[train_ratio + validate_ratio:],event_labels[train_ratio + validate_ratio:],args)
    test_loader = torch.utils.data.DataLoader(test_set, collate_fn=collate_fn_atomic_fine_tune, batch_size=16,shuffle=False)
