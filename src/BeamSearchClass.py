...
class BeamSearch(object):
    ...
    def __init__(self, args):
        ...
        self.event_net.load_state_dict(torch.load(args.event_model_path, encoding='latin1'))
        ...
        self.mind_trans_table = pickle.load(open(os.path.join(args.stat_path, 'mind_train.p')),encoding='latin1')  # todo: normalize
    ...
    def init(self, clip):
        ...
        with open(self.args.cate_path + clip.split('.')[0] + '/' + clip, 'rb') as f:
            self.category = pickle.load(f,encoding='latin1')

        # feature
        with open(os.path.join(self.args.data_path, 'feature_single', clip), 'rb') as f:
            self.feature_single = pickle.load(f,encoding='latin1')
        with open(os.path.join(self.args.data_path, 'feature_pair', clip), 'rb') as f:
            ...
            if len(possible_t_cp) >=1 and len(possible_b_cp) >=1:
                for cp_t_id in possible_t_cp[:search_N_cp_T]:
                    for cp_b_id in possible_b_cp[:search_N_cp_B]:
                        print("TREE LEVEL {}, parent node id:{}/{}, total len: {}-{}, cp_t:{}, cp_b:{}, time:{}".format(
                        ...
                        if len(test_seq) == 0:
                            ...
                            test_set = mydataset_atomic(test_seq, self.args)
                            test_loader = torch.utils.data.DataLoader(test_set, collate_fn=collate_fn_atomic,
                                                                      batch_size=16, shuffle=False)
                            test_results = test(test_loader, self.atomic_event_net, self.args)
                            ...
                            new_node2['T']['event'].append([1, outputs[0][3]])
                            new_node3['T']['event'].append([2, outputs[0][4]])
                        ...
                        if len(test_seq) == 0:
                            ...
                            norm_sum=outputs[0][0]+outputs[0][3]+outputs[0][4]

                            new_node1['B']['event'].append([0, outputs[0][0]/norm_sum])
                            new_node2['B']['event'].append([1, outputs[0][3]/norm_sum])
                            new_node3['B']['event'].append([2, outputs[0][4]/norm_sum])
                ...
                for cp_b_id in possible_b_cp[:search_N_cp_B]:
                    ...
                    if len(test_seq) == 0:
                        ...
                        new_node2['B']['event'].append([1, outputs[0][3]])
                        new_node3['B']['event'].append([2, outputs[0][4]])
