...
class BeamSearch(object):
    ...
    def init(self, clip):
        ...
        self.clip = clip
        self.clip_len = clips_len[clip]

        self.init_cps_T=list(range(0, self.clip_len, 5))
        self.init_cps_T.append(self.clip_len)
        self.init_cps_T=list(np.unique(self.init_cps_T))
        self.init_cps_B=list(range(0,self.clip_len,5))
...
def parse_arguments():
    ...
    parser.add_argument('--save-event-score', default='/media/lfan/HDD/NIPS20/data/event_score_per5frame/')
...
if __name__ == "__main__":
    ...
    args.save_path=op.join(args.project_path2, 'BestTree_per5frame_0531')
    ...
    Parallel(n_jobs=-1)(delayed(test_best_Tree)(atomic_event_net, event_net, clip_idx, clip, args) for clip_idx, clip in enumerate(mind_test_clips))
