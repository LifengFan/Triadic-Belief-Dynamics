...
class Atomic_node_only_lstm_first_view(torch.nn.Module):
    def __init__(self):
        super(Atomic_node_only_lstm_first_view, self).__init__()
        ...
        # print(modules)
        # modules.append(torch.nn.Linear(1000, 128))
        # modules.append(torch.nn.ReLU())
        # modules.append(torch.nn.Linear(128, 6))
        self.resnet50 = nn.Sequential(*modules)
        # print(list(self.resnet50.modules()))
        for p in list(self.resnet50.parameters()):
        ...
        #---------------- lstm fun ---------------------
        self.lstm=torch.nn.LSTM(input_size=48 + 60, hidden_size=48, batch_first=True,bidirectional=True)
        self.lstm.flatten_parameters()
        self.lstm_readout_fc1=torch.nn.Linear(self.nodefeat_n1*2, self.nodefeat_n1, bias=True)
        self.lstm_readout_fc2=torch.nn.Linear(self.nodefeat_n1, 7, bias=True)

        self.lstm_readout=torch.nn.Linear(self.nodefeat_n1, 12, bias=True)
        ...
        self.en_fc1=torch.nn.Linear(en_out_size,en_out_size//2)
        self.en_fc2=torch.nn.Linear(en_out_size//2,36)
        self.en_fc3=torch.nn.Linear(36,3)
    ...
    def forward(self, nodes, pos):
        ...
        pos = pos.view((N, sq_len, -1))
        nodes=nodes.view(N*sq_len*max_node_num, 3, 224, 224) # 8, 5, 6, 2048
        nodes_feature=self.resnet50(nodes).view(N*sq_len*max_node_num, -1) #.view(N, sq_len, -1) #16.5.4.6
        nodes_feature = self.fc1(nodes_feature)
        nodes_feature = self.relu(nodes_feature)
        nodes_feature = self.bn1(nodes_feature)
        nodes_feature = self.fc2(nodes_feature)
        nodes_feature = self.relu(nodes_feature)
        nodes_feature = self.bn2(nodes_feature)
        nodes_feature = self.fc3(nodes_feature).view(N, sq_len, -1)
        fc_input = torch.cat([nodes_feature, pos], 2).view((N, -1))

        en_out = self.en_fc1(fc_input)
        en_out = self.relu(en_out)
        en_out = self.bn3(en_out)
        en_out = self.en_fc2(en_out)
        en_out = self.relu(en_out)
        en_out = self.bn4(en_out)
        pred_label = self.en_fc3(en_out)
        return pred_label
