...
def get_gt_data():
    ...
    path=Path('home')
    clip_path = path.home_path2+'BestTree_ours_split_steps_0601/'
    ...
    save_path = path.save_root+'/mind_likelihood_ours_split_steps_0601/'
    ...
    for clip in clips:
        if not os.path.exists(reannotation_path + clip):
            continue
        with open(op.join(path.home_path2, 'BestTree_ours_split_steps_0601', clip), 'rb') as f:
            tree = pickle.load(f)
        ...
        for obj_name in obj_names:
            ...
            for frame_id in range(p1_events_by_frame.shape[0]):
                ...
                m1, m2, m12, m21, mc = net(event_input)
