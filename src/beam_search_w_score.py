            ...
            if len(possible_t_cp) >=1 and len(possible_b_cp) >=1:
                for cp_t_id in possible_t_cp[:search_N_cp_T]:
                    for cp_b_id in possible_b_cp[:search_N_cp_B]:
                        ...
                        with open(op.join(args.save_event_score, self.clip.split('.')[0], 'tracker', '{}_{}.p'.format(start, end)), 'rb') as f:
                        ...
                        with open(op.join(args.save_event_score, self.clip.split('.')[0], 'battery', '{}_{}.p'.format(start, end)), 'rb') as f:
                                outputs=pickle.load(f)
...
def finetune_para(atomic_net, event_net, clip_list, para_bank, args): #,pid):
...
def parse_arguments():
    ...
    parser.add_argument('--save-event-score', default=home_path2 + 'data/event_score/')
...
if __name__ == "__main__":
    ...
    Parallel(n_jobs=total_process)(delayed(finetune_para)(atomic_event_net, event_net, clips_with_gt_event, [para], args) for para in para_bank)
    ...
    #     project_name = 'beam_search_m_lfan_05191335_process_{}'.format(i)
    #     args.save_path = args.save_root + project_name
    #     if not os.path.exists(args.save_path):
    #         os.makedirs(args.save_path)
    #
    #     # clip_list = clips_with_gt_event[int((clip_len / total_process)* i):int((clip_len / total_process) * (i + 1))]
    #     # para_bank = para_bank[int((para_len / total_process) * i):int((para_len / total_process) * (i + 1))]
    #
