...
def get_gt_data():
    ...
    path=Path('home')
    clip_path = path.home_path2+'BestTree_init_seg_0531/'
    ...
    save_path = path.save_root+'/mind_likelihood_init_seg_0531/'
    ...
    for clip in clips:
        if not os.path.exists(reannotation_path + clip):
            continue
        with open(op.join(path.home_path2, 'BestTree_init_seg_0531', clip), 'rb') as f:
            tree = pickle.load(f)
