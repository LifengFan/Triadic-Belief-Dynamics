...
class Atomic_node_only_lstm_first_view(torch.nn.Module):
    ...
    def _load_pretrained_weight(self, model_path):
        ...
        pretrained_model = torch.load(model_path, encoding='latin1')['state_dict']
        # ---------------------------------------------------------------------
        # load resnet weight
        model_dict=self.resnet.state_dict()
        pretrained_dict={}
    ...
    def forward(self, nodes, pos, hog):
        # nodes [N, 5, 4, 3, 224, 224]
        # head [N, 5, 2, 3, 224, 224]
        # pos [N, 5, 4, 6]
        ...
        nodes=nodes.view(N*sq_len*max_node_num, 3, 224, 224)
        nodes_feature=self.resnet(nodes).view(N, sq_len, -1) #16.5.4.6
        hog_feature = hog.view((-1, hog.size()[-1]))
        ...
        hog_feature = hog_feature.view((hog.size()[0], hog.size()[1], -1))

        fc_input = torch.cat([nodes_feature, pos, hog_feature], 2).view((hog.size()[0], -1)) #todo
        # output, (h_n, _) = self.lstm(lstm_input)  # [1, sq_len,  2*262]

        # en_out = self.en_fc1(h_n.clone().view((-1, h_n.size()[-1]*2)))  # 1, 5, 48*2
