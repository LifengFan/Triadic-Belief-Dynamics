...
def get_raw_data():
    ...
    path=Path('home')
    clip_path = path.home_path2+'BestTree_0529/'
    ...
    obj_bbox_path=path.obj_bbox
    save_path = path.save_root+'mind_search_input/'
    ...
    obj_transforms = transforms.Compose(
        [transforms.Resize([224, 224]),
         ...
         transforms.Normalize(mean=[0.5, 0.5, 0.5],
                              std=[0.5, 0.5, 0.5])])
    for clip in clips:
        with open(clip_path + clip, 'rb') as f:
            tree = pickle.load(f)
        ...
        tracker_events_by_frame = seg2frame(tracker_seg)
        battery_events_by_frame = seg2frame(battery_seg)
        ...
        for obj_name in obj_names:
            key = './post_box_reid/' + clip.split('.')[0] + '/' + obj_name.split('/')[-1]
            if category[key][0] == 'person':
                continue
            print(obj_name)
            ...
            for frame_id in range(p1_events_by_frame.shape[0]):
                ...
                cv2.rectangle(img, (int(obj_bbox[0]), int(obj_bbox[1])),
                              (int(obj_bbox[2] + obj_bbox[0]), int(obj_bbox[3] + obj_bbox[1])), (255, 0, 0),
                              thickness=2)
                ...
                if not gt_obj_name:
                    mind_output_gt.append('NA')
                    mind_output.append('NA')
                    continue
                ...
                # obj_patch
                img_patch = img[int(obj_bbox[1]):int(obj_bbox[1] + obj_bbox[3]), int(obj_bbox[0]):int(obj_bbox[0] + obj_bbox[2])]
            ...
            if len(mind_output) > 0:
                with open(save_prefix + obj_name.split('/')[-1], 'wb') as f:
                    pickle.dump([mind_output, mind_output_gt], f)

def get_gt_data():

    path=Path('home')
    clip_path = path.home_path2+'BestTree_0529_init_seg/'
    reannotation_path = path.reannotation_path
    annotation_path = path.annotation_path
    color_img_path = path.img_path
    hog_path = path.feature_single
    cate_path = path.cate_path
    save_path = path.save_root+'/mind_search_input_init_seg/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    for clip in clips:
        if not os.path.exists(reannotation_path + clip):
            continue
        with open(op.join(path.home_path2, 'BestTree_0529_init_seg', clip), 'rb') as f:
            tree = pickle.load(f)
