...
class Atomic_node_only_lstm(torch.nn.Module):
    ...
    def forward(self, nodes, pos, attmat_gt):
        ...
        intermidate_output = torch.autograd.Variable(torch.zeros(N,36)).cuda()
        ...
        for pass_rnd in range(iterN):
            for b_id in range(N):
                ...
                if pass_rnd==(iterN-1):
                    ...
                    en_out1=self.relu(en_out)
                    pred_label[b_id, :]=self.en_fc3(en_out1)
                    intermidate_output[b_id, :] = en_out1
        ...
        #return en_out #[N,6]
        return pred_label, intermidate_output
