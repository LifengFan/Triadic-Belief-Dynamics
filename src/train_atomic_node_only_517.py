...
def main(args):
    ...
    #criterion = [torch.nn.CrossEntropyLoss(weight=torch.Tensor([1/800., 1/400.,1/400.])), torch.nn.MSELoss()]
    criterion = [torch.nn.CrossEntropyLoss(weight=torch.Tensor([0.2, 0.4, 0.4])), torch.nn.MSELoss()]
...
#--------------------------------------------------
    ...
    with open(os.path.join(args.save_test_res, 'raw_test_results.p'), 'wb') as f:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (head_batch, pos_batch, attmat_batch, index, hog_batch, atomic_label_batch) in enumerate(train_loader):
        ...
        with torch.set_grad_enabled(True):
            ...
            pred_atomic= model(heads, poses, hog) # [N,6]
...
def validate(validate_loader, model, criterion, epoch, args):
    ...
    for i, (head_batch, pos_batch, attmat_batch, index, hog_batch, atomic_label_batch) in enumerate(validate_loader):
...
def test(test_loader, model, criterion, args):
    ...
    for i, (head_batch, pos_batch, attmat_batch, index, hog_batch, atomic_label_batch) in enumerate(test_loader):
        ...
        with torch.set_grad_enabled(False):
            ...
            pred_atomic = model(heads, poses, hog) #[N, 6, 1,1,1]
...
def parse_arguments():
    ...
    parser.add_argument('--save-test-res', default='./fine_tune_event/',help='path to save test metrics')
...
if __name__ == '__main__':
    ...
    # the best atomic net model is named as "atomic_best.pth"
